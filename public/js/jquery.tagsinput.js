...
(function($) {
	...
	jQuery.fn.addTag = function(value,options) {
			...
			this.each(function() { 
				...
				if (value !='') { 
					...
					$('<span class="tag">'+value + '&nbsp;&nbsp;<a href="#" title="Удалить" onclick="return $(\'#'+id + '\').removeTag(\'' + escape(value) + '\');"></a></span>').insertBefore('#'+id+'_addTag');
					tagslist.push(value);
	...
	jQuery.fn.tagsInput = function(options) { 
		...
		var settings = jQuery.extend({defaultText:'Add +',width:'300px',height:'100px','hide':true,'delimiter':',',autocomplete:{selectFirst:false}},options);
